...
from src.instance_correspondence import mask_for
...
def dense(backend,image,obj):
    ...
    pixels=backend.processor(images=[crop],do_resize=False,do_center_crop=False,return_tensors='pt')['pixel_values']
    assert tuple(pixels.shape[-2:])==(crop.height,crop.width)
    with torch.no_grad(),torch.autocast(device_type='cuda',dtype=torch.float16):
        values=backend.model(pixel_values=pixels.to(backend.device)).last_hidden_state[0,1:]
    ...
    for y in range(gh):
        for x in range(gw):
            ...
            if np.count_nonzero(region)/region.size>=.75 and float(texture.std())>=8:
                keep.append(y*gw+x);points.append([box[0]+(x+.5)*patch,box[1]+(y+.5)*patch])
    if len(keep)<24:
        return None,'NATIVE_FOREGROUND_LOW_TEXTURE'
    take=np.linspace(0,len(keep)-1,min(512,len(keep))).astype(int)
    return {'vectors':vectors[np.asarray(keep)[take]],'points':np.asarray(points,dtype=np.float32)[take],
            'native_grid':[gw,gh],'native_patch_size':patch,'foreground_textured_patches':len(keep)},None


def compare(a,b,oa,ob):
    import cv2
    if a is None or b is None:
        return {'eligible':False,'reason':'NO_NATIVE_DESCRIPTORS','score':0.}
    sim=a['vectors']@b['vectors'].T
    columns=np.argmax(sim,axis=1);rows=np.argmax(sim,axis=0)
    source=[];target=[];scores=[]
    for i,j in enumerate(columns):
        value=float(sim[i,j])
        alternatives_row=np.partition(sim[i],-2)[-2]
        alternatives_col=np.partition(sim[:,j],-2)[-2]
        if rows[j]==i and value>=.9 and value-alternatives_row>=.01 and value-alternatives_col>=.01:
            source.append(i);target.append(j);scores.append(value)
    result={'eligible':False,'reason':'PATCH_RECIPROCAL_SUPPORT_LOW','score':0.,'matches':len(source),
            'source_patches':len(a['vectors']),'target_patches':len(b['vectors'])}
    if len(source)<12:
        return result
    p=a['points'][source];q=b['points'][target]
    matrix,inliers=cv2.estimateAffinePartial2D(p,q,method=cv2.RANSAC,ransacReprojThreshold=28,maxIters=2000)
    if matrix is None or inliers is None:
        return {**result,'reason':'PATCH_GEOMETRY_UNRESOLVED'}
    valid=inliers.ravel().astype(bool);n=int(valid.sum())
    areas=[(o['box'][2]-o['box'][0])*(o['box'][3]-o['box'][1]) for o in [oa,ob]]
    coverage=[float(np.prod(np.ptp(points[valid],axis=0))/area) if n else 0. for points,area in zip([p,q],areas)]
    scale=float(np.hypot(matrix[0,0],matrix[1,0]))
    fraction=n/min(len(a['vectors']),len(b['vectors']))
    eligible=n>=12 and n/len(source)>=.65 and min(coverage)>=.15 and .8<=scale<=1.25 and fraction>=.08
    return {**result,'eligible':bool(eligible),'reason':'NATIVE_PATCH_SUPPORT' if eligible else 'PATCH_DRIFT_OR_LOCAL_FRAGMENT',
            'inliers':n,'inlier_ratio':n/len(source),'coverage':coverage,'scale':scale,'score':fraction,
            'median_similarity':float(np.median(scores)),
            'source_inlier_centers':p[valid][:64].tolist(),'target_inlier_centers':q[valid][:64].tolist(),
            'whole_body_completeness':'unknown','identity_authority':False}
...
def main():
    ...
    group=next(g for g in json.loads((a.root/'slice.json').read_text())['groups'] if g['group_alias']=='G018')
    cfg=load_config(group['config']).as_dict();cfg['features'].update(iqa_musiq=False,iqa_clipiqa=False)
    ...
    for frame in group['frames'][:2]:
        ...
        original=json.loads((a.root/f'next-class-scale/2026-07-25-G018-{frame["alias"]}.json').read_text())
        obs=[];desc=[]
        frame_out=a.output/frame['alias'];frame_out.mkdir(exist_ok=True)
        for index,seed in enumerate(original['observations']):
            ...
            selected={**selected,'source_observation_index':index,'detector_confirmed':True,
                      'whole_body_completeness':'unknown','frame':frame['alias']}
            ...
            print(frame['alias'],index,'class',selected['class_id'],'patches',None if value is None else len(value['vectors']),why,flush=True)
    ...
    for pair in eligible:
        ...
        if ((not competitors or pair['score']>=max(competitors)*1.5)
                and _iou(objects[0][pair['source_index']]['box'],objects[1][pair['target_index']]['box'])>=.5):
            admitted.append(pair)
    result={'group':'G018','frames':['F01','F02'],'objects':objects,'pairs':pairs,'review_tracklet_candidates':admitted,
            'candidate_count':len(admitted),'class_or_human_identity_authority':False,'runtime_admitted':False,
            'full_four_frame_track_confirmed':False,'native_no_resize':True,
            'refusals':dict(Counter(p['reason'] for p in pairs if not p['eligible']))}
    save(a.output/'result.json',result)
    summary={k:v for k,v in result.items() if k not in ['objects','pairs','review_tracklet_candidates']}
