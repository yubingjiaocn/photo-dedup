...
from src.instance_correspondence import (view_agreement,masked_crop,foreground_keypoints,
                                         foreground_motion,match_appearance,track_components)
...
def choose(seed,observations):
    scores=sorted([(_iou(seed['box'],o['box']),i) for i,o in enumerate(observations)
                   if o['class_id'] in (0,77) and min(o['box'][2]-o['box'][0],o['box'][3]-o['box'][1])>=96],reverse=True)
    if not scores or scores[0][0]<.5:
        return None,'NO_INDEPENDENT_TARGET'
    if len(scores)>1 and scores[0][0]-scores[1][0]<.15:
...
def detect_view(detector,image,window,path):
    ...
    prediction=detector.predict(crop,imgsz=960,conf=.6,agnostic_nms=True,max_det=64,
                                device=0,save=False,verbose=False)[0]
    observations=[]
    if prediction.masks is not None:
        for box,cls,confidence,polygon in zip(prediction.boxes.xyxy.cpu().tolist(),prediction.boxes.cls.cpu().tolist(),prediction.boxes.conf.cpu().tolist(),prediction.masks.xy):
            edge=min(box[0],box[1],crop.width-box[2],crop.height-box[3])
            observations.append({'box':[box[0]+window[0],box[1]+window[1],box[2]+window[0],box[3]+window[1]],
                                 'class_id':int(cls),'confidence':float(confidence),
                                 'polygon':(polygon+np.array(window[:2])).tolist(),
                                 'crop_edge':edge<max(8,.01*min(crop.size)),
                                 'edge_distance_native_px':edge,'window':window})
...
def observe_frame(frame,image,seeds,prior,detector,out):
    ...
    w,h=image.size
    observations=[];attempts=[];potential=[]
    for i,seed in enumerate(seeds):
        box=seed['box'];bw=box[2]-box[0];bh=box[3]-box[1]
        views=[];reason=None
        for j,(mx,my) in enumerate([(.35,.25),(.7,.5)]):
            window=[max(0,int(box[0]-bw*mx)),max(0,int(box[1]-bh*my)),
                    min(w,int(box[2]+bw*mx)),min(h,int(box[3]+bh*my))]
            detections=detect_view(detector,image,window,out/f'view-{i:02d}-{j}.json')
            potential.extend(o for o in detections if o['class_id'] in (0,77)
                             and min(o['box'][2]-o['box'][0],o['box'][3]-o['box'][1])>=96)
            selected,reason=choose(seed,detections)
            if selected is None:
                break
            views.append(selected)
        evidence={'eligible':False,'reason':reason} if reason else view_agreement(*views,(h,w))
        attempts.append({'seed_index':i,'source_frame':seed['source_frame'],'seed_box':box,'views':views,'agreement':evidence})
        if not evidence['eligible']:
            continue
        # Same hypothesized object in two views: use the broader confirmed extent,
        # never choose the largest actor in a scene.
        selected=max(views,key=lambda o:(o['box'][2]-o['box'][0])*(o['box'][3]-o['box'][1]))
        selected={**selected,'view_confirmed':True,'view_evidence':evidence,'seed_index':i,
    ...
    for obj in sorted(potential,key=lambda o:o['confidence'],reverse=True):
        if novel_detection(obj,observations):
            observations.append({**obj,'view_confirmed':False,
                                 'new_vs_v0_catalog':novel_detection(obj,prior)})
    result={'frame':frame,'shape':[w,h],'observations':observations,'attempts':attempts,
            'refusals':dict(Counter(x['agreement']['reason'] for x in attempts if not x['agreement']['eligible']))}
    save(path,result)
    return result
...
def group_run(group,root,out,detector,backend):
    ...
    for frame in group['frames']:
        ...
        packet=json.loads((root/f'next-class-scale/{group["date"]}-{group["group_alias"]}-{frame["alias"]}.json').read_text())
        for o in packet['observations']:
            if o['class_id'] in (0,77) and not any(_iou(o['box'],s['box'])>=.8 for s in seeds):
    ...
    for index,(frame,image) in enumerate(zip(group['frames'],images)):
        ...
        vectors.append(v);records.append(record)
    links=[];refusals=Counter();edges=[]
    for s in range(len(records)):
        for t in range(s+1,len(records)):
            ...
            for r in rejected:
                refusals[r['reason']]+=1
                links.append({'source_frame':s,'target_frame':t,**r,'eligible':False})
            for pair in pairs:
                i,j=pair['source_index'],pair['target_index']
                support=foreground_motion(points[s][i],points[t][j],left[i],right[j])
                links.append({'source_frame':s,'target_frame':t,**pair,'support':support,'eligible':support['eligible']})
                if support['eligible']:
    ...
    for track in tracks:
        track['new_observations']=[{'frame':f,'index':i,'file_id':group['frames'][f]['id']}
                                   for f,i in enumerate(track['slots']) if i is not None and records[f]['observations'][i]['new_vs_v0_catalog']]
    proposed=[t for t in tracks if t['new_observations']]
    recovered={(x['frame'],x['index']) for t in proposed for x in t['new_observations']}
    for f,(image,record) in enumerate(zip(images,records)):
        ...
        for i,obj in enumerate(record['observations']):
            ...
            draw.text((box[0],box[1]),f'{i} cls{obj["class_id"]}',fill=color,stroke_width=1,stroke_fill='black')
        panel.save(out/f'{group["frames"][f]["alias"]}.jpg',quality=90)
    result={'day':group['date'],'group_alias':group['group_alias'],'group':group,'method':'independent_views_masked_dino_sift',
            'records':records,'links':links,'tracks':tracks,'proposed_tracks':proposed,
            'new_frame_observations':len(recovered),'refusals':dict(refusals),
            'complete_semantic_subject_set':'unknown','keeper_authority':False,'runtime_admitted':False}
    save(result_path,result)
    print(group['date'],group['group_alias'],'observations',[len(r['observations']) for r in records],
          'tracks',len(tracks),'new tracks',len(proposed),'new frames',len(recovered),dict(refusals),flush=True)
    return result
...
def main():
    ...
    groups=[g for g in json.loads((a.root/'slice.json').read_text())['groups'] if g['group_alias'] in a.groups]
    if len(groups)!=len(set(a.groups)):
        raise ValueError('Every requested group must match the frozen slice exactly once')
    ...
    results=[group_run(g,a.root,a.output/f'{g["date"]}-{g["group_alias"]}',detector,backend) for g in groups]
    summary={'groups':len(results),'frames':sum(len(g['group']['frames']) for g in results),
             ...
             'complete_semantic_subject_sets_confirmed':0,'keeper_changed_groups':0,
             'seconds':time.perf_counter()-started,'runtime_admitted':False,
