    ...
    for group in groups:
        ...
        for frame in group["frames"]:
            ...
            if result_path.exists():
                ...
                continue
            row = c.execute(
                "SELECT f.path,f.size_bytes,f.mtime_ns,fe.quality_meta FROM files f JOIN features fe ON f.id=fe.file_id WHERE f.id=?",
                (frame["id"],),
            ).fetchone()
            path = Path(row[0])
            stat = path.stat()
            ...
            for index, window in enumerate(windows):
                ...
                for box, cls, confidence in zip(
                    ...
                    if index and (
                        box[0] <= 2
                        or box[1] <= 2
                        or box[2] >= crop.width - 2
                        or box[3] >= crop.height - 2
                    ):
                        continue
                    b = [
                        box[0] + window[0],
            ...
            result = {
                ...
                "novel_box_candidates": novel,
                "identity_authority": False,
                "keeper_authority": False,
                "semantic_subject_authority": False,
            ...
            for index, o in enumerate(novel):
                ...
                draw.text(
                    (box[0], box[1]),
                    f"{index} {o['class_name']} {o['confidence']:.2f}",
                    fill=color,
                    stroke_width=1,
                    stroke_fill="black",
                )
            ...
            rows.append(result)
            print(
                stem, "novel boxes", Counter(o["class_name"] for o in novel), flush=True
            )
        c.close()
    summary = {
        "status": "bounded_exposed_class_scale_probe_complete",
        "frames": len(rows),
        "groups": len(groups),
        "novel_box_classes": dict(
            Counter(o["class_name"] for r in rows for o in r["novel_box_candidates"])
        ),
        "outside_v0_classes": sum(
            o["outside_v0_classes"] for r in rows for o in r["novel_box_candidates"]
        ),
        "identity_or_semantic_truth": False,
        "v0_modified": False,
        "model_installed": str(a.model),
