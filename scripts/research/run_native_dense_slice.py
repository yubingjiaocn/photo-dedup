...
from src.instance_correspondence import track_components  # noqa: E402
from src.local_quality import _iou  # noqa: E402
from probe_native_dense_tracks import dense, compare  # noqa: E402
...
def run_group(group, root, out, detector, backend):
    ...
    for f, frame in enumerate(group["frames"]):
        ...
        if row[0] != frame["path"] or (stat.st_size, stat.st_mtime_ns) != (
            row[1],
            row[2],
        ):
        ...
        print(
            group["group_alias"], frame["alias"], "observations", len(obs), flush=True
        )
    c.close()
    pairs = []
    admitted = []
    ...
    for s in range(len(objects)):
        for t in range(s + 1, len(objects)):
            ...
            for pair in eligible:
                ...
                pair["object_uniqueness_ratio"] = (
                    pair["score"] / max(competitors) if competitors else None
                )
                if (
                    not competitors or pair["score"] >= max(competitors) * 1.5
                ) and _iou(
                    objects[s][pair["source_index"]]["box"],
                    objects[t][pair["target_index"]]["box"],
                ) >= 0.5:
                    pair["source_new_vs_v0"] = objects[s][pair["source_index"]][
                        "new_vs_v0_catalog"
                    ]
                    pair["target_new_vs_v0"] = objects[t][pair["target_index"]][
                        "new_vs_v0_catalog"
                    ]
                    admitted.append(pair)
                    edges.extend(
                        [
                            ((s, pair["source_index"]), (t, pair["target_index"])),
                            ((t, pair["target_index"]), (s, pair["source_index"])),
                        ]
                    )
    ...
    new_nodes = {
        (p[side + "_frame"], p[side + "_index"])
        for p in proposed
        for side in ["source", "target"]
        if p[side + "_new_vs_v0"]
    }
    confirmed_nodes = {
        (p[side + "_frame"], p[side + "_index"])
        for p in admitted
    ...
    for f, (image, obs) in enumerate(zip(images, objects)):
        ...
        for i, o in enumerate(obs):
            color = (
                "#00ff80"
                if (f, i) in new_nodes
                else ("#80c0ff" if (f, i) in confirmed_nodes else "#ffc040")
            )
    ...
    result = {
        ...
        "objects": objects,
        "pairs": pairs,
        "accepted_pairs": admitted,
        "new_observation_pair_proposals": proposed,
        "tracks": tracks,
        "new_frame_observations": len(new_nodes),
        "complete_subject_set": "unknown",
...
def main():
    ...
    summary = {
        ...
        "new_observation_proposed_groups": sum(
            bool(g["new_observation_pair_proposals"]) for g in results
        ),
        "accepted_pair_proposals": sum(len(g["accepted_pairs"]) for g in results),
        "new_observation_pair_proposals": sum(
            len(g["new_observation_pair_proposals"]) for g in results
        ),
        ...
        "all_frames_observed_tracklets": sum(
            t["all_frames_observed"] for g in results for t in g["tracks"]
        ),
