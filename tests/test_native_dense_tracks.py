...
def sample():
    points=np.array([[125+x*45,105+y*38] for y in range(10) for x in range(10)],np.float32)
...
def test_dense_native_translation_is_supported_without_identity_authority():
    a=sample();b=sample();b['points']=b['points']+[7,-3]
...
def test_tiny_local_fragment_is_not_full_extent_support():
    a=sample();b=sample()
    a['points']/=20;b['points']/=20
    result=compare(a,b,box(),box())
    assert not result['eligible']
    assert max(result['coverage'])<.15


def test_capacity_abstains_instead_of_resizing_native_pixels():
    backend=SimpleNamespace(torch=None,model=SimpleNamespace(config=SimpleNamespace(patch_size=14)))
