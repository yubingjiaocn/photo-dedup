...
from src.instance_correspondence import (mask_for,view_agreement,masked_crop,
    foreground_keypoints,foreground_motion,match_appearance,track_components)


def obj(cls=0,dx=0):
    return {'class_id':cls,'box':[100+dx,80,450+dx,500],
            'polygon':[[100+dx,80],[450+dx,80],[450+dx,500],[100+dx,500]],
            'crop_edge':False,'view_confirmed':True}
...
def test_background_is_neutral_not_native_identity_evidence():
    ...
    o=obj();o['polygon']=[[200,150],[400,150],[400,400],[200,400]]
...
def test_class_switch_uses_appearance_not_class_or_position():
    a=[obj(0),obj(0,30)];b=[obj(77),obj(0,30)]
    accepted,refused=match_appearance(a,b,np.eye(2),np.eye(2))
    assert len(accepted)==2 and not refused
    assert accepted[0]['source_detector_class']==0 and accepted[0]['target_detector_class']==77


def test_unconfirmed_peer_still_blocks_manufactured_uniqueness():
    a=[obj(),obj(77)];b=[obj(77),obj()]
    a[1]['view_confirmed']=False;b[1]['view_confirmed']=False
...
def test_unconfirmed_target_abstains_even_with_unique_embedding():
    a=[obj()];b=[obj(77)];b[0]['view_confirmed']=False
...
def test_native_foreground_translation_needs_reciprocal_distributed_points():
    ...
    source=cv2.GaussianBlur(rng.integers(0,255,(600,600),dtype=np.uint8),(3,3),0)
    target=cv2.warpAffine(source,np.array([[1,0,10],[0,1,0]],np.float32),(600,600))
    a,b=obj(),obj(77,10)
    ...
    assert all(100<=x<=450 and 80<=y<=500 for x,y in result['source_inliers_native'])
