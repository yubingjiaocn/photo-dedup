...
The bookkeeping either side of the loop -- admission, the pending-work query, the
optional thumbnail/eye-detector construction, and the closing meta/stats -- lives
in :mod:`src.stage1_setup`, so this file stays about reading, decoding and turning
one batch into feature rows.
"""
...
def _run_with_db(conn: Any, cfg: Config, telemetry: Any, settings: Any,
    ...
    skipped, skip_counts = stage1_setup.admit(conn, cfg, scope, telemetry)
    thumbnailer = stage1_setup.build_thumbnailer(cfg, telemetry)
    pending = stage1_setup.pending_work(conn, thumbnailer, scope, telemetry, limit)
    total = len(pending)
    ...
    with telemetry.phase("setup_detectors"):
        eye_detector = stage1_setup.build_eye_detector(cfg, bool(pending))
