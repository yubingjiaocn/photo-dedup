...
def view_agreement(a, b, shape):
    ...
    miou = float(np.count_nonzero(ma & mb) / union) if union else 0.0
    area_ratio = min(np.count_nonzero(ma), np.count_nonzero(mb)) / max(
        1, max(np.count_nonzero(ma), np.count_nonzero(mb))
    )
    eligible = overlap >= 0.65 and miou >= 0.65 and area_ratio >= 0.75
    return {
        "eligible": bool(eligible),
        "reason": "INDEPENDENT_VIEW_CONFIRMED"
        if eligible
        else "VIEW_MASK_OR_EXTENT_UNSTABLE",
        "box_iou": overlap,
        "mask_iou": miou,
...
def foreground_motion(left, right, a, b):
    ...
    matches = [
        ...
        if len(pair) == 2
        for m, n in [pair]
        if m.distance < 0.75 * n.distance and back.get(m.trainIdx) == m.queryIdx
    ]
    if len(matches) < 12:
        return {
            "eligible": False,
            "reason": "FOREGROUND_RECIPROCAL_SUPPORT_LOW",
            "matches": len(matches),
        }
    p = np.float32([ka[m.queryIdx].pt for m in matches])
    q = np.float32([kb[m.trainIdx].pt for m in matches])
    matrix, inliers = cv2.estimateAffinePartial2D(
        p, q, method=cv2.RANSAC, ransacReprojThreshold=3, maxIters=2000
    )
    ...
    coverage = [
        float(np.prod(np.ptp(points[valid], axis=0)) / max(area, 1)) if n else 0.0
        for points, area in zip([p, q], areas)
    ]
    scale = float(np.hypot(matrix[0, 0], matrix[1, 0]))
    eligible = (
        n >= 12
        and n / len(matches) >= 0.65
        and min(coverage) >= 0.15
        and 0.8 <= scale <= 1.25
    )
    return {
        "eligible": bool(eligible),
        "reason": "NATIVE_FOREGROUND_SUPPORT"
        if eligible
        else "FOREGROUND_DRIFT_OR_LOCAL_PATCH",
        "matches": len(matches),
        "inliers": n,
        "inlier_ratio": n / len(matches),
        "coverage": coverage,
        "scale": scale,
        "source_inliers_native": p[valid][:64].tolist(),
        "target_inliers_native": q[valid][:64].tolist(),
...
def match_appearance(a, b, va, vb):
    ...
    for i, x in enumerate(a):
        ...
        row = np.delete(similarity[i], j)
        col = np.delete(similarity[:, j], i)
        margin = min(
            value - float(row.max()) if row.size else 2.0,
            value - float(col.max()) if col.size else 2.0,
        )
        info = {
            "source_index": i,
            "target_index": j,
            "similarity": value,
            "margin": margin,
            "source_detector_class": x["class_id"],
            "target_detector_class": b[j]["class_id"],
        }
        if value < 0.9:
            refusals.append({**info, "reason": "FOREGROUND_APPEARANCE_BELOW_FLOOR"})
        elif margin < 0.03 or int(np.argmax(similarity[:, j])) != i:
            refusals.append({**info, "reason": "FOREGROUND_APPEARANCE_NOT_UNIQUE"})
        elif _iou(x["box"], b[j]["box"]) < 0.5:
    ...
    for root in sorted(nodes):
        ...
        while todo:
            ...
            if node in component:
                continue
            component.add(node)
            todo.extend(adjacent[node] - component)
        seen |= component
        ...
        tracks.append(
            {
                "slots": slots,
                "all_frames_observed": all(i is not None for i in slots),
