...
Split out of :mod:`src.stage1_features` so that file stays about the loop —
read, decode, models, feature rows — and this one owns the bookkeeping around
it: admission of unprocessable files, the pending-work query, the optional
thumbnail/eye-detector construction, and the meta/stats written at the end.

Each step keeps its own telemetry phase, so a slow SSD listing or a slow
admission pass shows up as itself instead of as "unaccounted" time.

Nothing here runs off the main thread and nothing here reads a photo: every
function touches only the SQLite connection, the thumbnail cache, model
construction, or cheap inventory metadata. Originals stay read-only.
"""
